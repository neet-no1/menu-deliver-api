...
public class MenuController implements MenuApi {
    ...
    public ResponseEntity<BasicResponse> favoriteMenu(@Valid FavoriteMenuParam favoriteMenuParam) {
        ...
        // 献立IDの存在チェック
        if (favoriteMenuParam.getId() == null) {
            response.setCode(MenuDeliverStatus.FAILED);
            ErrorInfo error = new ErrorInfo();
            error.setErrorMessage("献立IDが空です。");
            response.setErrorInfo(error);
        ...
        // 追加・解除フラグの存在チェック
        if (favoriteMenuParam.isAdded() == null) {
            response.setCode(MenuDeliverStatus.FAILED);
            ErrorInfo error = new ErrorInfo();
        ...
        // お気に入りへの追加・解除を行う
        menuService.favoriteMenu(auth.getPrincipal().toString(), favoriteMenuParam.getId().intValue(),
                favoriteMenuParam.isAdded().booleanValue());
