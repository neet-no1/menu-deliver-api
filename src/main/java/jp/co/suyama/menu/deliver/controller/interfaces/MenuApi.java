...
public interface MenuApi {
    ...
    @ApiOperation(value = "食品成分表情報取得", nickname = "getCompositions", notes = "食品成分表のデータ一覧を取得する 認証不要 ", response = CompositionsResponse.class, tags={ "Menu", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "通信成功時の返却値", response = CompositionsResponse.class),
        @ApiResponse(code = 400, message = "200以外の時のは通信失敗をクライアントに通達") })
    @RequestMapping(value = "/menu/compositions",
        produces = { "application/json" }, 
    ...
    @RequestMapping(value = "/menu",
        ...
        method = RequestMethod.POST)
    ResponseEntity<BasicResponse> postMenu(@ApiParam(value = "献立ID　存在しない場合は0", required=true) @RequestParam(value="id", required=true)  Integer id,@ApiParam(value = "献立のタイトル", required=true) @RequestParam(value="title", required=true)  String title,@ApiParam(value = "内容", required=true) @RequestParam(value="contents", required=true)  String contents,@ApiParam(value = "作り方", required=true) @RequestParam(value="cookery", required=true)  String cookery,@ApiParam(value = "投稿/保存のフラグ", required=true) @RequestParam(value="opened", required=true)  Boolean opened,@ApiParam(value = "献立のサブタイトル") @RequestParam(value="subTitle", required=false)  String subTitle,@ApiParam(value = "献立のサムネイル画像") @Valid @RequestPart(value="thumb", required=false) MultipartFile thumb,@ApiParam(value = "カテゴリID") @RequestParam(value="category", required=false)  Integer category,@ApiParam(value = "ファイルリスト") @RequestParam(value="files", required=false)  List<byte[]> files,@ApiParam(value = "ファイルの説明") @RequestParam(value="filesDescription", required=false)  String filesDescription);
